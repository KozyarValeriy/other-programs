...
def get_dict_city(file_name: str) -> dict:
    """ Function to reading csv file with [city,cookie] and
...
def main():
    ...
    for city, cookie in cities_cookies.items():
        ...
        phones += [phone.replace(' ', '').replace('+', '').replace('-', '') for phone
                   in re.findall(pattern_phone_2, str(first))]
        phones = set(phone.replace('8', '7', 1) if phone.startswith('8') else phone for phone in phones)
